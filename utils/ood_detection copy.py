...
from utils.energy_detector import OODDetectionModule, evaluate_ood_metrics
...
def run_ood_detection(trainer, cfg, args, stage):
    ...
    detector = OODDetectionModule(
        model=trainer.model,
        temperature=float(args.ood_temperature),
        energy_alpha=float(args.ood_alpha),
    ...
    detector.calibrate_threshold(
        trainer.val_loader, percentile=float(args.ood_percentile), log_file=log_path
    )
    ...
    if ood_test is not None and len(ood_test) > 0:
        ...
        test_loader_for_ood = build_data_loader(
            ...
            tfm=base_tfm,          # 关键：让 DatasetWrapper 把 PIL 转成 tensor
            is_train=False,
        )
    else:
    ...
    auroc, aupr, fpr95 = evaluate_ood_metrics(
        detector, id_loader, ood_loader, log_file=log_path
    )

    msg = (
        f"[OOD Detection] AUROC={auroc:.6f}, AUPR={aupr:.6f}, FPR95={fpr95:.6f} "
        f"(T={float(args.ood_temperature):.4f}, alpha={float(args.ood_alpha):.4f}, pct={float(args.ood_percentile):.2f}), "
...
def extend_cfg(cfg):
    ...
    cfg.DATASET.NUM_OOD_SAMPLES = 5  # 默认 OOD 样本数
...
if __name__ == "__main__":
    ...
    parser.add_argument("--ood-alpha", type=float, default=0.7)
    parser.add_argument("--ood-percentile", type=float, default=90.0)
    parser.add_argument(
        "opts",
        default=None,
