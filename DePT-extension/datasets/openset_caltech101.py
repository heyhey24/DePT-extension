...
class OpenSetCaltech101(DatasetBase):

    dataset_dir = "caltech-101"

    def __init__(self, cfg):
        """Open-set variant of Caltech-101 with staged OOD logic.

        - 使用与原 Caltech101 相同的方式构建 train/val/test 划分；
        - 复用 OpenSetOxfordPets 中的 openset 流程（subsample_classes + _build_stage_split），
          以避免在多个数据集间复制 openset 逻辑。
        """

        # 1) Caltech-101 特有的数据读取与 few-shot 预处理
        self.cfg = cfg
        ...
        # 为 Caltech101 定义单独的 OOD 类索引文件路径
        ...
        if subsample == "base":
            ...
            # openset 流程：对 Caltech101 复用 OpenSetOxfordPets 的 staged 逻辑
            if current_stage == 0:
